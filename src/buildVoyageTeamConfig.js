...
const addVoyagerToTeam = (teams, currentTeamNo, voyagerCategory, voyager) => {
  ...
  const discordNames = team.discord_names
  const githubNames = team.github_names
...
const addTeamResourcesToTeam = (team, teamNo, voyagers) => {
  const findPOs = (voyager) => voyager.team_no === teamNo && voyager.role === 'Product Owner'
  const findUIUXs = (voyager) => voyager.team_no === teamNo && voyager.role === 'UI/UX'
  const findWebdevs = (voyager) => voyager.team_no === teamNo && voyager.role === 'Developer'
  const findDataScientists = (voyager) => voyager.team_no === teamNo && voyager.role === 'Data Scientist'
  const findVoyageGuides = (voyager) => voyager.team_no === teamNo && voyager.role === 'Voyage Guide'
  ...
  const voyageGuides = voyagers.filter(findVoyageGuides) ? voyagers.filter(findVoyageGuides)
    .map(voyager => '@'.concat(voyager.discord_name)) : 'None'
  const githubRepoURL = `https://github.com/chingu-voyages/v44-tier2-team-${ teamNo.padStart(2,0) }`
  const gdrivePlaceholderURL = 'https://drive.google.com/drive/folders/'

  const resourceMsg = [
    `**__${ voyagers[0].voyage.concat('-',voyagers[0].tier,'-team-',voyagers[0].team_no) } team:__**\n`,
...
const buildVoyageTeamConfig = async (environment, VOYAGE) => {
  ...
  config.teams.forEach(team => {
    addTeamResourcesToTeam(team, team.name.slice(-2), voyagers)
    console.log('team.resource_msg: ', team.resource_msg)
  })  
